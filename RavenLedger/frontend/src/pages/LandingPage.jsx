...
const FEATURES = [
  { tag: 'Triage',      h3: 'Top Risk Queue',          p: 'Every case ships with a ranking reason — why this case first, explained in plain language by the supervisor.' },
  { tag: 'Detection',   h3: 'Payment Fraud Investigation', p: 'Detect fraud labels, split invoice patterns, duplicate signals, and abnormal submission behavior.' },
  { tag: 'Correlation', h3: 'Insider Risk Correlation',   p: 'Connect suspicious users with device, HTTP, and logon behavior.' },
  { tag: 'Telemetry',   h3: 'Live Splunk Evidence',       p: 'Run SPL against BOTS v3 and attach evidence — with a plain-English explanation of every generated query.' },
  { tag: 'Governance',  h3: 'Named Controls Checklist',   p: 'CTRL-mapped policy decisions: supplier compliance, threshold bypass, approval requirements — passed, failed, or warning.' },
  { tag: 'Reporting',   h3: 'Audit-Ready Reports',        p: 'Generate reports that explain what happened, why it matters, and what action should be taken.' },
];
  ...
  return (
    <div>
        ...
        <div className={`${styles.wrap} ${styles.heroGrid}`}>
          <div>
            ...
            <div className={styles.heroCtas}>
              ...
              <Link to="/demo/capability" className={`${styles.btn} ${styles.btnGhost}`}>View Audit Report</Link>
            </div>
          </div>
      ...
      <section className={`${styles.converge} ${styles.section}`} id="converge">
        <div className={styles.wrap}>
          <span className={`${styles.eyebrow} ${styles.reveal}`} data-reveal="">Correlated investigation</span>
      ...
      <section className={`${styles.queue} ${styles.section}`} id="queue">
        <div className={styles.wrap}>
          ...
          <div className={`${styles.queuePanel} ${styles.reveal}`} data-reveal="">
            {QUEUE_ROWS.map((row) => (
              <div key={row.id} className={`${styles.qRow} ${row.top ? styles.qRowTop : styles.qRowDim}`}>
                <span className={styles.qRank}>{row.rank}</span>
                <span className={styles.qDot} style={{ background: row.dotColor }}/>
                <div>
                  <span className={styles.qId}>{row.id}</span>
                  <p className={styles.qReason}>
                    {row.reasonBold && <b>{row.reasonBold}</b>}
                    {row.reason}
                  </p>
                </div>
                <span className={`${styles.qPill} ${styles[row.pillCls]}`}>{row.pill}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className={styles.section}>
        <div className={styles.wrap}>
          <span className={`${styles.eyebrow} ${styles.reveal}`} data-reveal="">Capabilities</span>
          <h2 className={`${styles.featuresH2} ${styles.reveal}`} data-reveal="">See what RavenLedger can do for high-risk payments</h2>
          <p className={`${styles.featuresLead} ${styles.reveal}`} data-reveal="">RavenLedger turns scattered signals into a complete investigation: invoice risk, supplier context, insider activity, Splunk telemetry, policy violations, and human-approved response.</p>
          <div className={styles.featGrid}>
            {FEATURES.map((f) => (
              <div key={f.h3} className={`${styles.feat} ${styles.reveal}`} data-reveal="">
                <span className={styles.featTag}>{f.tag}</span>
                <h3 className={styles.featH3}>{f.h3}</h3>
                <p className={styles.featP}>{f.p}</p>
              </div>
            ))}
          </div>
        </div>
