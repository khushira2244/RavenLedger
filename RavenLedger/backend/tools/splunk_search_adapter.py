...
def run_splunk_search(
    ...
    if not SPLUNK_PASSWORD:
        raise SplunkSearchError(
            "SPLUNK_PASSWORD is missing. Set it as an environment variable before running."
        )

    search_url = f"{SPLUNK_HOST}/services/search/jobs"
    results_url_template = f"{SPLUNK_HOST}/services/search/jobs/{{sid}}/results"
...
def run_template_search(name: str, **kwargs) -> list[dict]:
    ...
    return run_splunk_search(spl=spl, count=kwargs.get("limit", 20))
...
if __name__ == "__main__":
    ...
    results = run_template_search("sample_security_events")
